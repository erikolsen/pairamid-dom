...
const RoleSettings = ({ roles, setRoles }) => {
  ...
  const updateRole = (data) => {
    axios
      .put(`${API_URL}/team/${teamId}/role/${data.id}`, data)
      .then((response) => {
        setRoles(
          roles.map((role) =>
  ...
  const addRole = () => {
    axios.post(`${API_URL}/team/${teamId}/role`).then((response) => {
      setRoles([...roles, response.data]);
    });
  };

  const deleteRole = (id) => {
    axios.delete(`${API_URL}/team/${teamId}/role/${id}`).then((response) => {
  ...
  return (
    <div>
      <h2>Roles</h2>
      <div className="grid grid-cols-2 md:grid-cols-4">{roleList}</div>
      <button
        data-cy="add-role"
        onClick={addRole}
