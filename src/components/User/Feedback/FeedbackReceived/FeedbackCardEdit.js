...
const FeedbackCardEdit = ({
  ...
  const onSave = (data, e) => {
    ...
    axios
      .post(`${API_URL}/feedbacks/${feedback.id}`, payload)
      .then((response) => {
        e.target.reset();
            ...
            <div className="absolute bottom-0 left-0 w-full">
              ...
              <div className="grid grid-cols-4">
                <p
                  ...
                  className={`hover:bg-red py-2 col-span-1 cursor-pointer font-bold text-xs text-center`}
                >
                  Delete
                </p>

                <p
                  onClick={() => duplicateFeedback(feedback)}
                  className={`hover:bg-gray-border py-2 col-span-1 cursor-pointer font-bold text-xs text-center`}
                >
                  Duplicate
                </p>
                ...
                <p
                  onClick={() =>
                    setEditing({ inProgress: false, updated: false })
                  }
                  className="hover:bg-gray-border py-2 col-span-1 cursor-pointer font-bold text-xs text-center"
                >
                  Cancel
                ...
                <input
                  type="submit"
                  value="Save"
                  className="hover:bg-green py-2 col-span-1 cursor-pointer font-bold text-xs text-center"
                />
              </div>
            </div>
          </div>
